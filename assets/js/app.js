...
(function () {
  ...
  function renderCompanyPrice() {
    ...
    const costingBlock = () => {
      ...
      const esthPc = +costing.allocEsth || 0;                        // now directly configurable
      const emp = +costing.empExpMonth || 0;
      const profit = +costing.profitPct || 0;
      const o1p = Math.max(1, +costing.o1p || 1), o1f = +costing.o1f || 0;
      const o2p = Math.max(1, +costing.o2p || 1), o2f = +costing.o2f || 0;
      ...
      // Base rows (market-independent): name, units/mo, landing.
      const base = items.filter((it) => (+it.unitUSD > 0)).map((it) => ({ name: it.name, u: avg6(it), landing: (it.unitUSD * usd * (1 + cust)) + (+it.transport || 0) })).filter((r) => r.u > 0).sort((a, b) => b.u - a.u);
      const autoU = base.reduce((s, r) => s + r.u, 0);               // computed 6-month avg units/month
      ...
      const assume = `<div class="cost-assume">
        ...
        <div class="muted-note" style="margin-top:10px"><b>Split of ${rup(emp)}/month:</b> Machine ${machPc}% = ${rup(machBucket)} · Celluma ${cellPc}% = ${rup(cellBucket)} · Esthemax ${esthPc}% = <b>${rup(esthBucket)}</b>${sumPc !== 100 ? ` <span class="cov-need">⚠ shares add to ${sumPc}%, not 100%</span>` : ""}. Boxes/month: <b>${Math.round(boxes).toLocaleString("en-IN")}</b>${(+costing.unitsMonth > 0) ? ` <span class="t-muted">(manual; auto avg is ${Math.round(autoU)})</span>` : " <span class=\"t-muted\">(auto 6-month avg — editable)</span>"} → operation cost <b>${rup(opexU)}/unit</b>. &nbsp;<b>Total cost = Landing + Operation.</b> Offers & 10%-off are calculated on the market's <b>MRP</b>. FX ${usd} · customs ${Math.round(cust * 100)}%.</div>
      </div>`;
      // One cost-vs-MRP table for a given market (Saloon / Derma).
      const mktTable = (mid) => {
        const MKT = (EP.markets && EP.markets[mid]) || {};
        const mktLabel = MKT.label || mid;
        const mrpMap = {};
        ...
        const body = base.map((r) => {
          const cost = r.landing + opexU, cur = curMrp(r.name);
        ...
        const totalRow = `<tr class="cprice-total">
          ...
          ${offCell(t1, tCost)}
          ${offCell(t2, tCost)}</tr>`;
        return `<div class="block" style="margin-top:16px"><h2 style="margin:0 0 6px">${esc(MKT.icon || "🧴")} ${esc(mktLabel)} — cost vs MRP</h2>
          <div class="callout"><b>Total cost</b> = Landing + Operation. <b>${esc(mktLabel)} MRP</b> = latest ${esc(mktLabel)}-market price list (incl. 18% GST). <b>Margin %</b> = (MRP − Total cost) ÷ MRP (red = loss). <b>10% off</b> = MRP − 10%. <b>${o1lbl}</b> / <b>${o2lbl}</b> = effective ₹/unit under each buy+free offer, on the MRP (each shows its margin %). TOTAL row = per-month value.</div>
  ...
  const WEEKLY_DEFAULTS = {
    ...
    "Finance": {
      ...
      mandatory: [
        { id: "fnwzoho", task: "Finance · Zoho Projects tracking rule — every duty below is run as a task in Zoho Projects. DAILY duty: create ONE task for the week (title + due date = Saturday), add a comment each day describing what was done, log the actual hours spent that day, and CLOSE the task on Saturday. WEEKLY duty: create one task for the week, do it, log the hours, and close it on completion. MONTHLY duty: create one task for the month, comment progress and log hours, and close it once filed. ASSIGNED / ad-hoc duty: open a task the moment it is assigned, comment progress, log hours, and close it on completion. Nothing is closed until the work is 100% done and the hours are logged.", time: "", priority: "High", remark: "Team rule — applies to every Finance duty (Sonal & Viisvesh)" },
        { id: "fnw1", task: "Sonal · Update the MIS", time: 0.5, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily what was done + log hours, close Saturday" },
        { id: "fnw2", task: "Sonal · Check and verify the previous day's invoices in Zoho", time: 0.5, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw3", task: "Sonal · Post the day's entries in Tally", time: 1, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw4", task: "Sonal · Review and reconcile Zoho expenses", time: 0.5, priority: "Medium", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw5", task: "Sonal · Prepare MOUs and quotations on request", time: 0.5, priority: "High", remark: "As needed · Zoho: open a task per request — comment + log hours, close on completion" },
        { id: "fnw6", task: "Sonal · Create ledgers and issue sales invoices for payment collection", time: 0.5, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw7", task: "Sonal · Follow up with doctors and the sales team on pending payments", time: 1, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily (who / how much) + log hours, close Saturday" },
        { id: "fnw8", task: "Sonal · Update payment status to the sales team and share discount details with management", time: 0.5, priority: "Medium", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw10", task: "Sonal · Update the machine sales data", time: 0.5, priority: "High", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw11", task: "Sonal · Update the Outstanding Dashboard", time: 0.5, priority: "High", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw12", task: "Sonal · Update the Expenses Dashboard", time: 0.5, priority: "High", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw13", task: "Sonal · Review the current-year sales pipeline", time: 0.5, priority: "Medium", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw14", task: "Sonal · Attend the sales meeting and the weekly management blocker call", time: 1.5, priority: "High", remark: "Weekly · Zoho: 1 weekly task — note points + log hours, close after the call" },
        { id: "fnw15", task: "Sonal · Update the Bigin payment tasks", time: 0.5, priority: "Medium", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw16", task: "Viisvesh · Process payments — update the vendor sheet, add advance comments in Zoho Expenses, store invoices, and release payments after approval", time: 1, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw17", task: "Viisvesh · Generate invoices and delivery challans, then update the invoice in Bigin — tag the salesperson for confirmation and complete the pipeline name (invoice name, invoice no., city, product)", time: 1, priority: "High", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw18", task: "Viisvesh · Download the Maruti report and chase the delivery details if not delivered", time: 0.5, priority: "Medium", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw19", task: "Viisvesh · Attend to phone calls, emails and messages", time: 0.5, priority: "Medium", remark: "Daily / ongoing · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw20", task: "Viisvesh · Add the docket details to the Bigin notes", time: 0.25, priority: "Medium", remark: "Daily · Zoho: 1 weekly task — comment daily + log hours, close Saturday" },
        { id: "fnw21", task: "Viisvesh · Prepare ledgers as required by Sonal", time: 0.5, priority: "Medium", remark: "As needed · Zoho: open a task per request — comment + log hours, close on completion" },
        { id: "fnw22", task: "Viisvesh · Update and maintain the Inventory Dashboard", time: 1, priority: "High", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw23", task: "Viisvesh · Review the Pondy team's employee expenses", time: 0.5, priority: "Medium", remark: "Weekly · Zoho: 1 weekly task — log hours, close on completion (by Saturday)" },
        { id: "fnw25", task: "Viisvesh · Attend the blocker call (Bhanu, Sonal, Arjun and Dhinesh)", time: 1, priority: "High", remark: "Weekly · Zoho: 1 weekly task — note points + log hours, close after the call" },
      ],
      monthly: [
        { id: "fnm1", task: "Sonal · Download the sales invoices and credit notes from Zoho for GSTR-1", time: 1, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close once filed" },
        { id: "fnm2", task: "Sonal · Prepare the invoice-wise and credit-note-wise GSTR-1 report", time: 1.5, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close once filed" },
        { id: "fnm3", task: "Sonal · Match the GSTR-1 data with Tally", time: 1, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close once filed" },
        { id: "fnm4", task: "Sonal · Send the required GST data to the CA", time: 0.5, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close once shared" },
        { id: "fnm5", task: "Sonal · Check the purchase invoices for TDS", time: 1, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close on completion" },
        { id: "fnm6", task: "Sonal · Prepare the TDS working", time: 1, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close on completion" },
        { id: "fnm7", task: "Sonal · Reconcile Tally with GSTR-2B", time: 1.5, priority: "High", remark: "Monthly · Zoho: 1 monthly task — comment progress + log hours, close on completion" },
      ],
      optional: [
        { id: "fno1", task: "Sonal · Maintain the machine pipeline data in Bigin", time: 0.5, priority: "Medium", remark: "Ongoing · Zoho: keep one open task — comment updates + log hours" },
        { id: "fno2", task: "Sonal · Complete the BOE reconciliation", time: 1, priority: "Medium", remark: "As needed · Zoho: open a task per BOE — comment + log hours, close on completion" },
        { id: "fno3", task: "Sonal · Complete additional work assigned by management or the CA", time: "", priority: "Low", remark: "When assigned · Zoho: open a task on assignment — comment + log hours, close on completion" },
        { id: "fno4", task: "Viisvesh · Prepare MOUs, payment receipts and quotations", time: 0.5, priority: "Medium", remark: "When assigned · Zoho: open a task on assignment — comment + log hours, close on completion" },
        { id: "fno5", task: "Viisvesh · Visit the office or bank", time: 1, priority: "Low", remark: "When required · Zoho: log a task with the purpose + hours on the day" },
        { id: "fno6", task: "Viisvesh · Create remittance forms", time: 0.5, priority: "Medium", remark: "When instructed · Zoho: open a task on instruction — comment + log hours, close on completion" },
        { id: "fno7", task: "Viisvesh · Prepare PTP letters and allocated tasks (employee expenses, installation-date updates)", time: 0.5, priority: "Medium", remark: "When allocated · Zoho: open a task per item — comment + log hours, close on completion" },
        { id: "fno8", task: "Viisvesh · Store the BOE when a shipment is received", time: 0.5, priority: "Medium", remark: "On shipment receipt · Zoho: log a task with hours when received" },
        { id: "fno9", task: "Viisvesh · Store the installation reports and update the new-machine sheet", time: 0.5, priority: "Medium", remark: "On dispatch / installation · Zoho: log a task with hours when the report is shared" },
      ],
    },
    "Admin & Logistics": {
